...
(function(window, navigator, Object) {
        ...
        _didDisconnect: function DNPad__didDisconnect() {
            ...
            for( i = 0; i < buttonDownTime.length; ++i ) {
                if( buttonDownTime[i] !== 0 ) {
                    self.fire('buttonup',   { 
                        target:self, 
                        controllerIndex: raw.index, 
                        button: i, 
                        value: rawValue, 
                        time: buttonDownTime[i] 
                    });
                    buttonDownTime[i] = 0;
                }
            }
    ...
    var Gamepads = EventTarget.inheritWith({
        ...
        __addPad: function Gamepads___addPad(rawPad) {
            ...
            var pad = dnPads[idx];
            ['buttondown', 'buttonup', 'axisdown', 'axisup'].forEach( function(type) {
                pad.on(type, function(ev) { ev.gamepad = pad; self.fire(type, ev); });
            } );
            self.fire('gamepadconnected', {gamepad: pad});
        },
